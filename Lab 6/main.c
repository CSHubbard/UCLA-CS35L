//  EDITED by Cody Hubbard to enable multi-threading
//	CS35L 004843389 
...
float printVals[width][height][3];

//global for number of threads
int nthreads;

void *DrawPixels(void* thread)
{
    ...
    {  
        ...
        for( int py=0; py<height; ++py )
        {
			...
			//add values to our data structure
...
main( int argc, char **argv )
{
	...
	int temp[nthreads];
	for (int n1=0; n1 < nthreads; n1++)
		temp[n1] = n1;
	
	int h;
	for (h=0; h < nthreads; h++)		
		pthread_create(&tid[h], NULL, DrawPixels, &temp[h]);
